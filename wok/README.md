...
9. Use `Mod+P` and run `Reset Settings` to restore the managed default config.
10. Use `Mod+Shift+S` to save the `manual` session snapshot, then `Mod+Shift+R` to load it.
