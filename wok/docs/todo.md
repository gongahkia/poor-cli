# Wok — A Warp-Inspired Terminal Emulator
...
**Workspace crates:**
- `wok-app`: entry point, event loop, window management (winit + wgpu)
- `wok-renderer`: glyph atlas, text rendering pipeline, damage tracking (wgpu)
- `wok-terminal`: thin wrapper around alacritty_terminal + portable-pty
- `wok-blocks`: block detection, shell integration, git info
- `wok-input`: gap buffer editor, syntax highlighting, command history
- `wok-ui`: tabs, splits, search, status bar, theming
...
**WHAT TO DO**
1. Create the project scaffold: `cargo init wok`, set up a Cargo workspace in `/Cargo.toml` with member crates `wok-app`, `wok-renderer`, `wok-terminal`, `wok-blocks`, `wok-input`, `wok-ui`.
2. In `wok-app/src/lib.rs`, integrate `winit 0.30+` as the windowing backend. Create a `WokWindow` struct wrapping `winit::window::Window` with fields: `title: String`, `size: PhysicalSize<u32>`, `scale_factor: f64`, `is_focused: bool`.
3. Implement `WokWindow::new(config: &WindowConfig) -> Result<Self, PlatformError>` that creates a resizable, titled window with a minimum size of 400x300 pixels. — WHY 400x300: smallest usable terminal size that fits ~40 cols x ~15 rows at default font, preventing layout breakage.
4. In `wok-app/src/event_loop.rs`, create `fn run_event_loop(window: WokWindow, mut app: impl AppHandler)` that starts the `winit` event loop and dispatches `AppHandler` trait methods: `on_redraw()`, `on_resize(new_size)`, `on_key_event(KeyEvent)`, `on_mouse_event(MouseEvent)`, `on_focus_change(bool)`, `on_close_requested()`.
5. Define the `AppHandler` trait in `wok-app/src/handler.rs` with all six methods above, each with default no-op implementations.

**DONE WHEN**
- [x] `cargo build --workspace` succeeds with zero errors on macOS, Linux, and Windows.
- [x] Running the binary opens a native window with the title "Wok" that responds to resize, focus, and close events (verified by logging each event to stdout).
...
**WHAT TO DO**
1. In `wok-app/src/frame_clock.rs`, create `FrameClock` struct with fields: `target_fps: u32`, `last_frame: Instant`, `frame_count: u64`, `accumulated_time: Duration`.
...
**PURPOSE** — Input event normalization: converts raw `winit` key events into a Wok-internal `KeyAction` enum so downstream modules (input editor, keybindings) work with a stable, platform-agnostic type.

**WHAT TO DO**
1. In `wok-app/src/input.rs`, define `KeyAction` enum with variants: `Char(char)`, `Enter`, `Tab`, `Backspace`, `Delete`, `Escape`, `ArrowUp`, `ArrowDown`, `ArrowLeft`, `ArrowRight`, `Home`, `End`, `PageUp`, `PageDown`, `FunctionKey(u8)`, `Copy`, `Paste`, `Cut`, `SelectAll`, `Undo`, `Redo`.
...
**WHAT TO DO**
1. In `wok-app/src/dpi.rs`, create `ScaleContext` struct: `{ scale_factor: f64, physical_size: PhysicalSize<u32>, logical_size: LogicalSize<f64> }`.
2. Implement `ScaleContext::from_window(window: &WokWindow) -> Self`.
...
**WHAT TO DO**
1. In `wok-renderer/src/gpu.rs`, create `GpuContext` struct holding `wgpu::Device`, `wgpu::Queue`, `wgpu::Surface`.
...
**WHAT TO DO**
1. Add `cosmic-text` as a dependency in `wok-renderer/Cargo.toml`.
...
**WHAT TO DO**
1. In `wok-renderer/src/atlas.rs`, create `GlyphAtlas` struct with a `wgpu::Texture` (e.g., 2048x2048 RGBA). — WHY 2048x2048: fits ~4000 ASCII glyphs at 16px, covers most terminal use cases without resize; 1024x1024 would overflow with CJK/emoji fallback fonts, 4096x4096 wastes VRAM on GPUs with small texture budgets.
...
**WHAT TO DO**
1. In `wok-renderer/src/pipeline.rs`, create the wgpu render pipeline with vertex and fragment shaders.
...
**WHAT TO DO**
1. In `wok-renderer/src/compositor.rs`, implement a compositor that blends layers: background color/image -> terminal cell backgrounds -> terminal glyphs -> UI overlays (selection, search highlights, block decorations).
...
**WHAT TO DO**
1. In `wok-renderer/src/damage.rs`, maintain a `DirtyRegion` bitmap (one bit per cell). — WHY per-row granularity as first pass: simpler than per-cell, and most terminal updates affect full rows (scrolling, line editing); per-cell tracking adds O(cols) bookkeeping with marginal savings.
...
**WHAT TO DO**
1. Create `wok-renderer/src/text_layout.rs`. Define `GlyphRun { glyphs: Vec<PositionedGlyph> }` and `PositionedGlyph { atlas_entry: AtlasRegion, x: f32, y: f32, color: Color }`.
...
**WHAT TO DO**
1. Add `alacritty_terminal` as a dependency in `wok-terminal/Cargo.toml`.
...
**WHAT TO DO**
1. Add `portable-pty` as a dependency in `wok-terminal/Cargo.toml`.
...
**WHAT TO DO**
1. In `wok-terminal/src/async_io.rs`, create `PtyIoHandle` struct: `{ writer: Arc<Mutex<PtyWriter>>, rx: Receiver<PtyEvent>, _reader_thread: JoinHandle<()> }`.
...
**WHAT TO DO**
1. In `wok-terminal/src/terminal.rs`, create `Terminal` struct: `{ state: TerminalState, pty: PtyIoHandle, title: String, dirty: bool }`.
...
**WHAT TO DO**
1. In `wok-terminal/src/config.rs`, create `TerminalConfig` struct exposing configurable options that map to alacritty_terminal's config:
...
2. Implement `TerminalConfig::to_alacritty_config(&self) -> alacritty_terminal::Config` that maps Wok's config to alacritty_terminal's internal config struct.
...
4. Ensure alternate screen buffer handling works correctly for programs like vim, less, htop (alacritty_terminal handles this, but verify the grid switching works with Wok's rendering).
...
**WHAT TO DO**
1. Create `wok-terminal/src/shell.rs`. Define `ShellType` enum: `Bash`, `Zsh`, `Fish`, `PowerShell`, `Wsl(String)` (String = distro name).
2. Implement `fn detect_default_shell() -> ShellType`:
   ...
   - On Windows: default to `PowerShell`. Check registry `HKCU\Software\Wok\DefaultShell` for overrides.
...
6. Embed all scripts as `include_str!()` constants in `wok-terminal/src/shell_integration.rs`. Implement `fn integration_args(shell: &ShellType) -> Vec<String>` that returns the extra args needed to inject the integration script.

**DONE WHEN**
- [x] Shell commands in Wok emit OSC 133 markers. Terminal can detect command boundaries.
- [x] In a Wok+zsh session, `cd /tmp` causes `\033]7;file://<host>/tmp\007` to appear in the PTY output.
...
2. Define `SemanticEvent` enum in `wok-blocks/src/semantic.rs`: `PromptStart { line: u16 }`, `CommandStart { line: u16 }`, `OutputStart { line: u16 }`, `CommandEnd { line: u16, exit_code: Option<i32> }`, `CwdChanged(PathBuf)`.
...
**DONE WHEN**
- [x] Running `ls` in a Wok+bash session emits the sequence: `PromptStart` -> `CommandStart` -> `OutputStart` -> `CommandEnd { exit_code: Some(0) }`.
...
**WHAT TO DO**
1. Create `wok-blocks/src/block.rs`. Define `Block` struct: `{ id: u64, prompt_text: String, command_text: String, output_start_line: usize, output_end_line: usize, exit_code: Option<i32>, start_time: Instant, end_time: Option<Instant>, is_collapsed: bool, scroll_offset: usize }`.
...
**WHAT TO DO**
1. In `wok-blocks/src/block_nav.rs`, implement `BlockNavigator` struct: `{ selected_block_index: Option<usize>, block_manager: BlockManager }`.
...
**WHAT TO DO**
1. In `wok-blocks/src/block.rs`, extend `Block` with fields: `duration: Option<Duration>` (computed from `end_time - start_time`), `git_branch: Option<String>`, `git_dirty: Option<bool>`, `cwd: PathBuf`.
2. When `CommandEnd` fires, compute duration and store it.
3. Implement `fn detect_git_info(cwd: &Path) -> Option<GitInfo>` where `GitInfo { branch: String, is_dirty: bool }`: wok up from CWD, find `.git/HEAD`, parse branch name. Check `.git/status` or run lightweight git status check.
...
**WHAT TO DO**
1. In `wok-blocks/src/block_search.rs`, create `BlockSearch` struct: `{ query: String, matches: Vec<SearchMatch>, current_match: usize, target_block_id: u64 }` where `SearchMatch { line: usize, col_start: usize, col_end: usize }`.
...
**WHAT TO DO**
1. Create `wok-input/src/buffer.rs`. Implement `InputBuffer` struct using a gap buffer: `{ text: Vec<char>, gap_start: usize, gap_end: usize, cursors: Vec<Cursor> }` where `Cursor { position: usize, anchor: Option<usize> }` (anchor is set when there's a selection; the selection range is `min(position, anchor)..max(position, anchor)`).
...
**WHAT TO DO**
1. In `wok-input/src/cursor_ops.rs`, implement cursor movement functions that operate on `InputBuffer`:
...
**WHAT TO DO**
1. Create `wok-input/src/highlighter.rs`. Define `HighlightSpan { start: usize, end: usize, kind: SpanKind }` and `SpanKind` enum: `Command`, `Argument`, `Flag`, `Path`, `String`, `Number`, `Pipe`, `Redirect`, `Variable`, `Comment`, `Error`, `Default`.
...
**WHAT TO DO**
1. In `wok-input/src/brackets.rs`, define bracket pairs: `('(', ')')`, `('[', ']')`, `('{', '}')`.
...
**WHAT TO DO**
1. In `wok-input/src/editor.rs`, create `InputEditor` struct: `{ buffer: InputBuffer, highlighter: fn, scroll_offset: usize, position: InputPosition, history: CommandHistory, is_active: bool }` where `InputPosition` enum: `Top`, `Bottom`.
...
**WHAT TO DO**
1. In `wok-input/src/history.rs`, create `CommandHistory` struct: `{ entries: Vec<String>, cursor: usize, history_file: PathBuf, max_entries: usize, search_query: Option<String>, search_results: Vec<usize> }`.
2. Implement `CommandHistory::load(path: &Path, max: usize) -> Self`: read history file (one command per line, most recent last). Default path: `~/.wok_history`. Default max: 10,000 entries. — WHY 10,000 max entries: balances file size (~500KB for typical commands) against recall depth; matches shell history defaults (bash HISTSIZE=1000 is often too low, 10K covers weeks of use).
...
- [x] History persists across application restarts (file exists at `~/.wok_history`).
...
**WHAT TO DO**
1. Create `wok-ui/src/layout/mod.rs`. Define `LayoutNode` enum:
   ```rust
   enum LayoutNode {
...
**WHAT TO DO**
1. In `wok-ui/src/tabs.rs`, create `TabManager` struct: `{ tabs: Vec<Tab>, active_tab: usize }` where `Tab { id: u64, title: String, terminal: Terminal, is_dirty: bool }`.
...
**WHAT TO DO**
1. In `wok-ui/src/splits.rs`, define a binary tree structure:
   ```rust
   enum SplitNode {
       Leaf { tab_id: u64 },
...
**WHAT TO DO**
1. In `wok-ui/src/viewport.rs`, create `ViewportRenderer` struct: `{ scroll_position: f32, smooth_scroll_target: f32, selection: Option<TextSelection> }` where `TextSelection { start: (u16, u16), end: (u16, u16) }`.
...
**WHAT TO DO**
1. In `wok-ui/src/tab_bar.rs`, create `TabBarRenderer`.
...
**WHAT TO DO**
1. In `wok-ui/src/status_bar.rs`, create `StatusBarRenderer`.
...
4. Implement `fn detect_git_branch(cwd: &Path) -> Option<String>`: wok up from cwd, find `.git/HEAD`, parse `ref: refs/heads/<branch>`.
...
**WHAT TO DO**
1. Create `wok-ui/src/theme.rs`. Define `Theme` struct with fields:
   ```rust
   pub struct Theme {
       pub name: String,
...
**WHAT TO DO**
1. Add `toml = "0.8"` and `serde = { version = "1", features = ["derive"] }` to `wok-ui/Cargo.toml`.
2. In `wok-ui/src/theme_loader.rs`, implement `fn load_theme(path: &Path) -> Result<Theme, ThemeError>`:
...
**WHAT TO DO**
1. Add `notify = "6"` to `wok-ui/Cargo.toml` (cross-platform filesystem watcher).
2. In `wok-ui/src/theme_watcher.rs`, create `ThemeWatcher` struct: `{ watcher: RecommendedWatcher, rx: Receiver<notify::Result<Event>>, active_path: PathBuf }`.
...
**DONE WHEN**
- [x] Editing and saving the theme TOML file while Wok is running causes the UI to update colors within 1 second.
...
**WHAT TO DO**
1. In `wok-ui/src/background.rs`, create `BackgroundRenderer` struct: `{ texture: Option<wgpu::Texture>, image_size: (u32, u32) }`.
...
**PURPOSE** — Configuration file system: loads, validates, and provides typed access to all Wok settings from a TOML config file.

**WHAT TO DO**
1. In `wok-app/src/config.rs`, define `WokConfig` struct:
   ```rust
   ...
   ```
2. Implement `WokConfig::load() -> Self`:
   - Search for config file at: `$WOK_CONFIG`, then `~/.config/wok/config.toml`, then `~/.wok.toml`.
   - Parse TOML, deserialize with `serde`, merge with `WokConfig::default()`.
   - If no config file found, use defaults.
3. Implement `WokConfig::default()`: sensible defaults (14px JetBrains Mono, auto-detected shell, 10000 scrollback, block cursor, bottom input).
4. Implement `WokConfig::config_dir() -> PathBuf`: `~/.config/wok/` on Unix, `%APPDATA%\Wok\` on Windows.

**DONE WHEN**
- [x] `WokConfig::load()` with no config file returns defaults without error.
...
**WHAT TO DO**
1. In `wok-app/src/keybindings.rs`, define `Action` enum with all bindable actions:
   ```rust
   pub enum Action {
...
**WHAT TO DO**
1. In `wok-ui/src/zoom.rs`, create `ZoomManager` struct: `{ base_size: f32, current_size: f32, min_size: f32, max_size: f32, step: f32 }`.
...
**PURPOSE** — Detect and support popular prompt frameworks (Starship, Powerlevel10k, Oh-my-Posh) so their styled prompts render correctly in Wok.

**WHAT TO DO**
1. In `wok-terminal/src/prompt.rs`, create `PromptDetector`.
...
**PURPOSE** — System clipboard integration: enables copy and paste between Wok and other applications on all platforms.

**WHAT TO DO**
1. Add `arboard = "3"` to `wok-ui/Cargo.toml` (cross-platform clipboard crate).
2. In `wok-ui/src/clipboard.rs`, create `ClipboardManager` struct wrapping `arboard::Clipboard`.
...
**WHAT TO DO**
1. In `wok-ui/src/selection.rs`, create `SelectionManager` struct: `{ state: SelectionState, last_click: Option<(Instant, Point)> }` where `SelectionState` enum: `None`, `Selecting { start: CellPos, end: CellPos }`, `Selected { start: CellPos, end: CellPos }` and `CellPos { row: u16, col: u16 }`.
...
**WHAT TO DO**
1. In `wok-ui/src/search.rs`, create `GlobalSearch` struct: `{ query: String, matches: Vec<GlobalMatch>, current: usize, is_active: bool, search_input: String }` where `GlobalMatch { screen_row: i64 (negative = scrollback), col_start: u16, col_end: u16 }`.
...
**WHAT TO DO**
1. In `wok-app/src/app.rs`, create `WokApp` struct:
   ```rust
   pub struct WokApp {
       config: WokConfig,
       theme: Theme,
   ...
   ```
2. Implement `WokApp::new(config: WokConfig) -> Result<Self, AppError>`: initialize all subsystems in dependency order. Create default tab with detected shell.
3. Implement `AppHandler` for `WokApp`:
...
**DONE WHEN**
- [x] `WokApp::new(default_config)` creates a running app with one tab, one terminal, and a visible prompt.
...
**WHAT TO DO**
1. In `wok-app/src/main.rs` (the root crate):
   ```rust
   fn main() -> Result<(), Box<dyn Error>> {
       let config = WokConfig::load();
       let window_config = WindowConfig { title: "Wok", width: 1200, height: 800 };
       let window = WokWindow::new(&window_config)?;
       let gpu = GpuContext::new(&window)?;
       let mut app = WokApp::new(config, gpu)?;
...
5. Set up logging: use `tracing = "0.1"` with `tracing-subscriber` for structured logging. Default level: `warn` in release, `debug` in debug builds. Log to `~/.config/wok/wok.log`.

**DONE WHEN**
- [x] `cargo run` launches Wok with a functional terminal.
...
**WHAT TO DO**
1. Add `mlua` crate with `lua54` and `serialize` features to `wok-app/Cargo.toml`. — WHY Lua 5.4: latest stable Lua version; 5.4 adds integers (useful for exit codes, line numbers) and generational GC (lower latency for interactive use). WHY mlua: safe Rust bindings, async support, serde integration; rlua is unmaintained, hlua lacks features.
2. Create `wok-app/src/scripting/mod.rs` with `LuaRuntime` struct wrapping `mlua::Lua`.
3. Initialize Lua VM on app startup, loading user config from `~/.config/wok/init.lua`.
4. Register a `wok` global table exposing: `wok.config`, `wok.keybindings`, `wok.theme`, `wok.on(event, callback)`.
...
- [x] `wok.config` table is readable from Lua.
...
**WHAT TO DO**
1. In Lua, expose `wok.keymap(mode, key, action)` where mode is `"normal"|"input"|"search"`.
...
4. Example: `wok.keymap("normal", "ctrl+shift+t", "new_tab")`
5. Example with Lua function: `wok.keymap("normal", "ctrl+shift+e", function() wok.exec("echo hello") end)`
...
**WHAT TO DO**
1. Expose `wok.theme.set(theme_table)` where `theme_table` has: `background`, `foreground`, `cursor`, `selection`, `ansi_colors` (16), and `ui_colors` (`tab_bar`, `status_bar`, `border`).
2. Expose `wok.theme.load(name)` to load a named theme from `~/.config/wok/themes/`.
3. Theme changes apply immediately (hot-reload).

**DONE WHEN**
- [x] `wok.theme.set({background = "#1e1e2e"})` changes the background immediately.
- [x] `wok.theme.load("catppuccin")` loads and applies a theme file.
...
**WHAT TO DO**
1. Expose `wok.on(event, callback)` for events: `"tab_created"`, `"tab_closed"`, `"command_finished"`, `"directory_changed"`, `"block_created"`.
2. Callback receives an event table with context (e.g., `{exit_code = 0, duration_ms = 1234}` for `command_finished`).
3. Example: `wok.on("command_finished", function(e) if e.exit_code ~= 0 then wok.notify("Command failed!") end end)`
...
**PURPOSE** — macOS application bundle: packages Wok as a `.app` bundle with an icon, Info.plist, and code signing support.

**WHAT TO DO**
1. Create `packaging/macos/` directory.
2. Create `Info.plist` with: `CFBundleName: Wok`, `CFBundleIdentifier: dev.wok.terminal`, `CFBundleVersion: 0.1.0`, `CFBundleExecutable: wok`, `CFBundleIconFile: wok.icns`, `LSMinimumSystemVersion: 10.15`, `NSHighResolutionCapable: true`.
3. Create a build script `packaging/macos/bundle.sh` that: runs `cargo build --release`, creates `Wok.app/Contents/MacOS/wok`, copies binary, copies `Info.plist` to `Contents/`, copies `wok.icns` to `Contents/Resources/`.
4. Create a placeholder `wok.icns` (can be generated from a PNG using `iconutil`).
5. Add a Makefile target: `make bundle-macos`.

**DONE WHEN**
- [x] `make bundle-macos` produces `Wok.app` that launches by double-clicking on macOS.
...
2. Create `wok.desktop` file: `[Desktop Entry]`, `Name=Wok`, `Exec=wok`, `Icon=wok`, `Type=Application`, `Categories=System;TerminalEmulator;`.
3. For `.deb`: create `packaging/linux/build_deb.sh` using `cargo-deb` or manual `dpkg-deb`. Include: binary at `/usr/bin/wok`, desktop file at `/usr/share/applications/wok.desktop`, icon at `/usr/share/icons/hicolor/256x256/apps/wok.png`, man page stub at `/usr/share/man/man1/wok.1.gz`.
4. For AppImage: create `packaging/linux/build_appimage.sh` using `linuxdeploy` with the AppDir structure: `AppDir/usr/bin/wok`, `AppDir/usr/share/applications/wok.desktop`, `AppDir/wok.png`.
5. Add Makefile targets: `make deb`, `make appimage`.

**DONE WHEN**
- [x] `make deb` produces a `.deb` file that installs with `dpkg -i` and `wok` is available in PATH.
- [x] `make appimage` produces a `.AppImage` that runs on a fresh Ubuntu system.
- [x] `wok.desktop` causes Wok to appear in the application launcher.
...
2. Create a WiX-based or Inno Setup installer script (`wok_installer.iss` or `wok.wxs`) that: installs `wok.exe` to `%PROGRAMFILES%\Wok\`, adds to PATH, creates Start Menu shortcut, registers as a terminal emulator.
...
4. For `winget` manifest: create `packaging/windows/winget/Wok.Wok.yaml` with installer metadata.
5. Add Makefile target: `make installer-windows` (runs via PowerShell or cross-compilation).

**DONE WHEN**
- [x] The installer installs Wok and it's accessible from the Start Menu.
- [x] `wok.exe` is in PATH after installation.
...
4. Add basic integration test in `tests/integration/`: spawn Wok in headless mode (no window), send input to PTY, verify output.
...
**WHAT TO DO**
1. Create `wok-input/tests/buffer_tests.rs` with unit tests for all `InputBuffer` operations:
...
2. Create `wok-input/tests/cursor_tests.rs` with unit tests for all cursor movement operations:
...
**DONE WHEN**
- [x] `cargo test -p wok-input` passes with all tests green.
...
**WHAT TO DO**
1. Create `wok-blocks/tests/block_detection_tests.rs`.
...
**DONE WHEN**
- [x] `cargo test -p wok-blocks` passes with all tests green.
...
**WHAT TO DO**
1. Create `wok-terminal/tests/pty_integration.rs`.
2. Spawn a real PTY with `/bin/sh` (POSIX shell for portability).
3. Send `echo "WOK_TEST_MARKER"\n` to PTY.
...
5. Scan the terminal grid for "WOK_TEST_MARKER" text.
...
- [x] "WOK_TEST_MARKER" is found in the terminal grid.
...
**WHAT TO DO**
1. Create `wok-renderer/tests/snapshot_tests.rs`.
...
4. Compare against reference images stored in `wok-renderer/tests/snapshots/` using pixel-diff with a tolerance of 2% (WHY 2%: accounts for sub-pixel rendering differences across platforms/font engines while catching visible regressions like wrong colors or missing glyphs).
...
**DONE WHEN**
- [x] `cargo test -p wok-renderer` passes with snapshot comparisons.
...
2. Name assets consistently: `wok-<version>-<os>-<arch>.<ext>` (e.g., `wok-v0.1.0-linux-x86_64.tar.gz`).
...
**PURPOSE** — Provides a comprehensive README and user-facing documentation so new users can install, configure, and use Wok effectively.
...
1. Create `README.md` with sections:
   ...
   - Installation: Homebrew (macOS), `cargo install wok-terminal`, AppImage (Linux), winget (Windows), build from source.
...
- [x] Configuration reference documents every field in `WokConfig` and `Theme`.
...
**PURPOSE** — Provides a Lua scripting guide so users can extend Wok with custom keybindings, themes, and event hooks.

**WHAT TO DO**
1. Create `docs/LUA_SCRIPTING.md` with sections:
   - Overview: what Lua scripting enables, where `init.lua` lives, how to reload.
   - API reference: document every function in the `wok` global table:
     - `wok.config` — reading and setting config values.
     - `wok.keymap(mode, key, action)` — full keybinding API with mode explanations.
     - `wok.theme.set(table)` — theme table schema with all fields.
     - `wok.theme.load(name)` — loading named themes.
     - `wok.on(event, callback)` — all events and their context tables.
     - `wok.exec(command)` — running shell commands.
     - `wok.notify(message)` — displaying status bar messages.
...
**WHAT TO DO**
1. In `wok-ui/src/bell.rs`, implement `BellHandler` with config option: `BellStyle { Visual, Audible, None }`.
...
**WHAT TO DO**
1. In `wok-ui/src/links.rs`, implement `fn detect_urls(line_text: &str) -> Vec<UrlSpan>` where `UrlSpan { col_start: u16, col_end: u16, url: String }`.
...
3. In the app handler, update `WokWindow::set_title(title)` (via `winit::window::Window::set_title`).
...
3. When mouse reporting is active, disable Wok's own selection (the application handles its own mouse input).

**DONE WHEN**
- [x] Opening `vim` in Wok and clicking positions the vim cursor at the clicked cell.
- [x] `tmux` mouse support works: clicking panes switches focus, scrolling works.
- [x] When mouse reporting is disabled (exiting vim), Wok's selection behavior resumes.
...
**WHAT TO DO**
1. In `wok-app/src/session.rs`, define `SessionState`:
   ```rust
   ...
   ```
2. Implement `fn save_session(app: &WokApp) -> Result<(), SessionError>`: serialize `SessionState` to JSON, write to `~/.config/wok/session.json`.
...
**DONE WHEN**
- [x] Closing Wok with 3 tabs and reopening restores 3 tabs with the correct CWDs.
...
**PURPOSE** — Regex triggers turn passive terminal output into an actionable surface. When a pattern matches block output, Wok fires an action: highlight, notify, bookmark the block, open a URL, or invoke a Lua hook. This is the highest-impact power-user feature across iTerm2 and Kitty, and it deepens Wok's block paradigm from "view output" to "react to output."

**WHAT TO DO**
1. Create `wok-blocks/src/triggers.rs`. Define `TriggerAction` enum: `Highlight { color: String }`, `Notify { message: String }`, `BookmarkBlock`, `OpenUrl`, `CopyMatch`, `LuaHook { hook_name: String }`. Define `Trigger` struct: `{ name: String, pattern: Regex, actions: Vec<TriggerAction>, scope: TriggerScope }` where `TriggerScope` is `Output | Command | Both`.
...
3. In `wok-blocks/src/block.rs`, call `TriggerEngine::evaluate` when `BlockManager` transitions out of `InOutput` state (i.e., on `SemanticEvent::CommandEnd`). Collect `TriggerMatch` results and emit them as a new `SemanticEvent::TriggersFired(Vec<TriggerMatch>)`.
4. In `wok-app/src/config.rs`, add a `[[triggers]]` TOML config section: `name`, `pattern` (regex string), `actions` (array of action descriptors), `scope` (default `"output"`).
5. In `wok-app/src/scripting.rs`, expose `wok.add_trigger(name, pattern, actions_table)` and `wok.remove_trigger(name)` in the Lua API. Actions table mirrors the TOML structure.
6. In `wok-app/src/action_effects.rs`, handle `TriggersFired` by dispatching each action: highlights are stored on the block for rendering, notifications go through `RuntimeEffect::Notify`, bookmarks toggle the block's bookmark flag, URLs open via `open::that()`, copies go to clipboard, Lua hooks invoke the registered callback.
...
- [x] `wok.add_trigger("ip", "\\d+\\.\\d+\\.\\d+\\.\\d+", { "copy_match" })` in `init.lua` works at runtime.
...
4. In `wok-app/src/keybindings.rs`, add `Action::QuickSelect` and `Action::QuickSelectBlock`. Add `Context::QuickSelect` with bindings: typing a label character narrows/selects, `Escape` cancels, `Enter` copies the selected match.
...
- [x] Custom patterns can be added via `wok.quick_select.add_pattern(name, regex)` in Lua.
...
**PURPOSE** — OSC 8 hyperlink support. Modern CLI tools (`ls --hyperlink`, `cargo`, `rustc`, `grep --hyperlink`, `gcc`) emit OSC 8 escape sequences to mark text as clickable links. Without support, Wok either swallows the escapes silently or renders garbage. This is table-stakes for a modern terminal.

**WHAT TO DO**
1. In `wok-terminal/src/terminal.rs`, extend `scan_osc_markers` (or the equivalent OSC dispatch) to detect OSC 8 sequences: `\e]8;params;uri\e\\` (link start) and `\e]8;;\e\\` (link end). Parse `params` as semicolon-separated `key=value` pairs (the `id` param groups non-contiguous link spans).
2. Maintain a `HyperlinkState` in `wok-terminal/src/state.rs`: `current_hyperlink: Option<HyperlinkInfo>` where `HyperlinkInfo { uri: String, id: Option<String>, params: HashMap<String, String> }`. When a link-start OSC 8 is received, set `current_hyperlink`. When link-end is received, clear it. All cells written while `current_hyperlink` is `Some` inherit the hyperlink reference.
...
4. In `wok-ui/src/links.rs`, unify the hyperlink model: `DetectedLink` (regex-found URLs) and `ExplicitLink` (OSC 8) both implement a `Linkable` trait with `fn uri(&self) -> &str` and `fn span(&self) -> CellRange`.
...
**WHAT TO DO**
1. Create `wok-input/src/workflows.rs`. Define `Workflow` struct: `{ name: String, description: String, template: String, params: Vec<WorkflowParam> }` where `WorkflowParam` is `{ name: String, placeholder: String, default: Option<String>, description: String }`. The template uses `$1`, `$2`, etc. or named `${branch}`, `${host}` placeholders.
2. Define `WorkflowStore` struct holding `Vec<Workflow>` with methods: `load_from_dir(dir: &Path)` (reads `~/.config/wok/workflows/*.toml`), `add(workflow)`, `remove(name)`, `find(query: &str) -> Vec<&Workflow>` (fuzzy match on name + description).
3. TOML format for workflow files:
   ...
   ```
4. In `wok-input/src/editor.rs`, add a `ParameterFillMode` state: when active, the input editor highlights the next unfilled `${...}` placeholder, Tab advances to the next placeholder, Shift+Tab goes back, Enter submits when all are filled.
5. In `wok-app/src/scripting.rs`, expose `wok.register_workflow(table)` and `wok.workflows()` in the Lua API.
6. Integrate workflows into the command palette (Task 78) as a dedicated section.

**DONE WHEN**
- [x] A TOML file in `~/.config/wok/workflows/deploy.toml` appears in the command palette under "Workflows".
...
- [x] `wok.register_workflow({ name = "quick-test", template = "cargo test $1", params = {{ name = "filter" }} })` in `init.lua` registers a workflow at runtime.
...
**PURPOSE** — Transform the command palette from a simple input mode into a full fuzzy-searchable dropdown showing all bindable actions, Lua-registered commands, workflows, recent commands, and file paths. This is the discoverability surface for Wok's entire feature set.

**WHAT TO DO**
1. Create `wok-ui/src/command_palette.rs`. Define `PaletteEntry` struct: `{ label: String, description: String, category: PaletteCategory, score: f64, action: PaletteAction }`. `PaletteCategory` enum: `Action`, `LuaCommand`, `Workflow`, `RecentCommand`, `FilePath`. `PaletteAction` enum mirrors what happens on selection.
...
4. Fuzzy scoring: reuse `fuzzy_score` from `wok-input/src/history.rs`. Score against `label` and `description`. Sort by category priority (workflows first if query matches, then actions, then recent commands) with score as tiebreaker.
...
5. In `wok-input/src/editor.rs`, add `CompletionPopupState`: `{ items: Vec<CompletionItem>, selected: usize, is_visible: bool, anchor_col: usize }`. Tab triggers completion (or cycles if popup is visible), Escape dismisses, Enter/Tab accepts the selected item.
...
**PURPOSE** — Vi mode for terminal output navigation. Wok already has block navigation; Vi mode fills the gap for character-level output navigation that power users expect. Block-aware custom motions (`[b` / `]b`) make this uniquely Wok's.

**WHAT TO DO**
1. Create `wok-ui/src/vi_mode.rs`. Define `ViModeState` struct: `{ cursor_row: usize, cursor_col: usize, mode: ViSubMode, visual_anchor: Option<(usize, usize)> }` where `ViSubMode` is `Normal | Visual | VisualLine | VisualBlock`.
...
3. Implement Wok-specific block motions: `[b` (jump to previous block start), `]b` (jump to next block start), `[e` (jump to previous block end), `]e` (jump to next block end). These use `BlockManager::blocks()` to find block boundaries by absolute row.
...
5. In `wok-app/src/keybindings.rs`, add `Context::ViMode` with all the motion bindings. Add `Action::EnterViMode` (default binding: `Mod+Shift+V` or configurable). Exiting: `Escape` in normal vi mode, or `q`, or `i` (returns to shell input).
6. Render the vi cursor as a distinct block cursor (different color from the PTY cursor, theme-configurable: `theme.vi_cursor_color`). In visual mode, render selection highlight using `SelectionManager` from `wok-ui/src/selection.rs`. Show a mode indicator in the status bar ("-- VI --", "-- VISUAL --", etc.).
...
**PURPOSE** — Pane broadcast input sends keystrokes to all panes simultaneously. This is a highly-valued tmux feature for running the same command on multiple servers or test environments. Trivial to implement given Wok's workspace architecture.

**WHAT TO DO**
1. In `wok-app/src/workspace.rs`, add a `broadcast_input: bool` field to `WorkspaceState` (default `false`).
2. In `wok-app/src/keybindings.rs`, add `Action::ToggleBroadcast` with default binding `Mod+Shift+B` (note: check for conflict with block bookmark toggle — if conflict, use `Mod+Alt+B` instead).
...
5. Broadcast only applies to raw input (keystrokes, paste). Wok-internal actions (block navigation, search, tab switching) remain pane-local — they should NOT be broadcast.
...
- [x] Wok-internal keybindings (Mod+Up for block nav, Mod+F for search) are NOT broadcast.
...
**WHAT TO DO**
1. Create `wok-terminal/src/sixel.rs`. Implement a Sixel parser: detect DCS (Device Control String) sequences starting with `\ePq` (or the full `\eP<params>q` form). Parse Sixel parameters: `Pn1` (aspect ratio), `Pn2` (background), `Pn3` (grid size).
...
**WHAT TO DO**
1. In `wok-ui/src/status_bar.rs`, extend `StatusBarState` (or the equivalent) with `custom_left: Vec<StatusSegment>`, `custom_center: Vec<StatusSegment>`, `custom_right: Vec<StatusSegment>` where `StatusSegment` is `{ text: String, fg: Option<String>, bg: Option<String>, bold: bool }`.
...
3. In `wok-app/src/scripting.rs`, expose a `wok.status_bar` Lua table with methods: `set_left(segments_table)`, `set_center(segments_table)`, `set_right(segments_table)`, and `clear()`. Each segment in the table is `{ text = "...", fg = "#rrggbb", bg = "#rrggbb", bold = true/false }`.
...
5. In `wok-app/src/plugin_host.rs`, drain status bar effects from the Lua runtime each frame tick so updates appear immediately, not on the next hook fire.

**DONE WHEN**
- [x] In `init.lua`: `wok.status_bar.set_right({{ text = " K8s: prod ", fg = "#00ff00" }})` shows a green "K8s: prod" segment on the right side of the status bar.
- [x] `wok.on("status_bar_refresh", function() wok.status_bar.set_left({{ text = os.date("%H:%M") }}) end)` shows a live-updating clock.
- [x] Multiple segments with different colors render correctly side-by-side.
- [x] `wok.status_bar.clear()` removes all custom segments.
...
**PURPOSE** — Streaming output hooks let Lua receive output lines from the active block in real time, not just on block completion. This enables building progress parsers, error detectors, live dashboards, and custom trigger logic entirely in Lua — turning Wok into a programmable terminal surface.

**WHAT TO DO**
1. In `wok-blocks/src/block.rs`, when `BlockManager` is in the `InOutput` state and processes new terminal lines, emit a new event type. Define it as a variant in the event pipeline (e.g., extend `SemanticEvent` or use a separate callback channel).
2. The event payload: `{ block_id: u64, line_number: usize, text: String }` where `line_number` is relative to the block's output start.
3. In `wok-app/src/scripting.rs`, add a new hook `output_line`. Document that this hook fires for every line of output as it arrives. The callback receives `{ block_id, line_number, text }`.
4. Performance guard: in `wok-app/src/plugin_host.rs`, only invoke the Lua callback if at least one listener is registered for `output_line`. Track registration count. If zero listeners, skip the entire output line processing path.
...
**DONE WHEN**
- [x] `wok.on("output_line", function(e) if e.text:match("FAIL") then wok.notify("Test failure detected!") end end)` shows a notification the instant a failing test line appears, before the command completes.
...
**WHAT TO DO**
1. In `wok-app/src/scripting.rs`, add `wok.set_timeout(ms, callback)` returning a `timer_id: u64`, and `wok.set_interval(ms, callback)` also returning a `timer_id`. Add `wok.clear_timer(timer_id)` to cancel either type.
...
3. In `wok-app/src/plugin_host.rs`, each frame tick (already running at 60fps), drain all due timers from the heap: pop entries where `fire_at <= Instant::now()`, invoke their callbacks. For intervals, re-insert with `fire_at = now + interval`. For timeouts, drop the `RegistryKey`.
4. Safety: limit the number of timer fires per frame tick to 64 to prevent runaway intervals from starving the render loop.

**DONE WHEN**
- [x] `wok.set_timeout(5000, function() wok.notify("5 seconds elapsed") end)` fires the notification after 5 seconds.
- [x] `local id = wok.set_interval(1000, function() print("tick") end)` prints "tick" every second. `wok.clear_timer(id)` stops it.
- [x] 100 simultaneous intervals at 100ms each do not cause frame drops (per-frame cap works).
- [x] Timer callbacks can call any other `wok.*` API (run_action, notify, status_bar, etc.).
...
**WHAT TO DO**
1. In `wok-app/src/workspace.rs`, extend `WorkspaceTab` with `floating_panes: Vec<FloatingPane>` where `FloatingPane` has `{ pane_id: PaneId, rect: Rect, z_order: u32, is_visible: bool, title: String }`. `Rect` uses logical pixel coordinates relative to the window.
...
3. Layout: floating panes render on top of the split tree. During layout computation in `wok-ui/src/layout.rs`, floating pane rects are overlaid after split tree rects are computed. Floating panes clip to the window bounds.
...
**WHAT TO DO**
1. Create `wok-ui/src/layout_presets.rs`. Define `LayoutPreset` struct: `{ name: String, description: String, tree: PresetNode }` where `PresetNode` is either `Leaf { weight: f64 }` or `Split { direction: Direction, ratio: f64, children: Vec<PresetNode> }`.
...
5. In `wok-app/src/keybindings.rs`, add `Action::NextLayout` and `Action::PrevLayout`. Default bindings: `Mod+Alt+L` (next), `Mod+Alt+Shift+L` (prev). Cycle through the preset list wrapping around.
...
**PURPOSE** — Named sessions with attach/detach decouple the terminal workspace from the window. This is tmux's core value proposition. Wok can run as a background daemon keeping PTYs alive, and the window becomes a thin client that connects via Unix socket. Users can close the window and reconnect later without losing their work. This eliminates the need for tmux inside Wok.

**WHAT TO DO**
1. Create `wok-app/src/daemon.rs`. The daemon is a headless Wok process that owns all PTY lifetimes, workspace state, and block managers. It listens on a Unix socket (Linux/macOS: `$XDG_RUNTIME_DIR/wok/<session-name>.sock`) or named pipe (Windows).
2. Create `wok-app/src/ipc.rs`. Define a message protocol: `ClientMessage` (key events, resize, paste, action requests, detach) and `ServerMessage` (terminal grid updates, block events, state snapshots, render instructions). Use length-prefixed JSON or MessagePack frames.
3. CLI changes: `wok` (no args) starts in standalone mode (current behavior). `wok --daemon <name>` starts a named daemon. `wok attach <name>` connects a window client to a running daemon. `wok list` shows running sessions. `wok kill <name>` terminates a session.
4. Refactor PTY ownership: in daemon mode, `wok-terminal::Terminal` lives in the daemon process. The client receives rendered grid state, not raw PTY output. The daemon processes PTY output, updates `TerminalState`, runs `BlockManager`, evaluates triggers, and sends diffs to the client.
...
6. Session naming: unnamed sessions auto-name as `wok-<pid>`. Named sessions persist across daemon restarts if the daemon is gracefully stopped (serialize state, respawn PTYs on restart).
...
**DONE WHEN**
- [x] `wok --daemon work` starts a background daemon named "work" with a shell.
- [x] `wok attach work` opens a window connected to the "work" session. Commands typed in the window execute in the daemon's PTY.
- [x] Closing the window (or `wok detach`) disconnects without killing the shell. The daemon keeps running.
- [x] `wok attach work` reconnects and shows the full terminal state including scrollback and blocks.
- [x] `wok list` shows `work (1 pane, attached)` or `work (1 pane, detached)`.
- [x] Multiple clients can attach to the same session (read-only viewers or collaborative input based on config).
- [x] `wok` with no args still works in standalone mode (no daemon).
...
**WHAT TO DO**
1. In `wok-terminal/src/state.rs`, add `kitty_keyboard_flags: u32` to `TerminalState`. The Kitty protocol defines flag bits: `0x1` (disambiguate escape codes), `0x2` (report event types), `0x4` (report alternate keys), `0x8` (report all keys as escape codes), `0x10` (report associated text).
2. In `wok-terminal/src/terminal.rs`, detect the mode query sequence `CSI ? u` (query current flags) and mode set `CSI > <flags> u` (push flags) and `CSI < u` (pop flags). Maintain a flag stack (applications push on enter, pop on exit).
3. In `wok-app/src/input.rs`, when `kitty_keyboard_flags > 0`, encode key events using the Kitty format instead of legacy xterm encoding: `CSI <unicode-key-code> ; <modifiers> u` for normal keys, with extensions for `event_type` (press=1, repeat=2, release=3) when flag `0x2` is set. Modifiers encoding: `shift=1, alt=2, ctrl=4, super=8`.
...
**DONE WHEN**
- [x] Opening Neovim (with `vim.o.termguicolors = true` and Kitty protocol support) inside Wok shows correct behavior for all key combinations including `Ctrl+Shift+P`, `Ctrl+I` vs `Tab`, and modifier-only events.
...
**PURPOSE** — Remote control protocol lets external scripts control Wok programmatically: create panes, send text, query state, change themes, trigger actions. This enables editor integration (Neovim `:terminal`), CI pipelines, and automation that goes beyond what Lua can do from inside Wok.

**WHAT TO DO**
1. Create `wok-app/src/remote_control.rs`. On startup, create a Unix socket at `$XDG_RUNTIME_DIR/wok-<pid>.sock` (or `$TMPDIR/wok-<pid>.sock` as fallback). On Windows, use a named pipe `\\.\pipe\wok-<pid>`.
2. Define a JSON-RPC 2.0 protocol. Methods map to the existing `Action`/`RuntimeEffect` bus:
   - `wok.run_action(action_name, params)` — dispatch any action
   - `wok.send_text(pane_id, text)` — write text to a pane's PTY
   - `wok.get_panes()` — list all panes with metadata
   - `wok.get_blocks(pane_id)` — list blocks for a pane
   - `wok.get_text(pane_id, start_row, end_row)` — read terminal text
   - `wok.create_pane(options)` — create a new pane
   - `wok.close_pane(pane_id)` — close a pane
   - `wok.set_theme(theme_name_or_path)` — change theme
   - `wok.notify(message)` — show a notification
...
4. Set `$WOK_SOCKET` environment variable in spawned shells so child processes can discover the socket path.
5. Security: the socket is user-readable only (`chmod 600`). No authentication beyond filesystem permissions.

**DONE WHEN**
- [x] `echo '{"jsonrpc":"2.0","method":"wok.get_panes","id":1}' | socat - UNIX-CONNECT:$WOK_SOCKET` returns a JSON list of panes.
- [x] `wok.send_text(pane_id, "ls\n")` from an external script executes `ls` in the target pane.
- [x] `wok.run_action("SplitVertical")` creates a new vertical split from an external script.
- [x] The `$WOK_SOCKET` variable is available in spawned shells.
...
3. Create `wok-ui/src/replay_overlay.rs`. When replay mode is active, render from the selected snapshot instead of live terminal state. Show a timeline scrubber at the bottom: a horizontal bar with block completion markers as tick marks. The user scrubs with left/right arrow keys or clicks on the timeline.
