...
mod imp {
    ...
    /// TCP-loopback JSON-RPC server for Windows remote control.
    ...
    impl PlatformServer {
        /// Start a non-blocking server on a loopback TCP port, writing port to a file.
        pub fn bind_default() -> Result<Self, std::io::Error> {
            let listener = TcpListener::bind(("127.0.0.1", 0))?;
            listener.set_nonblocking(true)?;
            let port = listener.local_addr()?.port();
            let endpoint_path = default_endpoint_path();
        ...
        /// Return the endpoint file path (contains the TCP port).
        pub fn socket_path(&self) -> &Path {
            &self.endpoint_path
        }
        ...
        pub fn poll_requests(&mut self) -> Vec<RemoteRequest> {
            ...
            if !remove_ids.is_empty() {
                self.clients.retain(|c| !remove_ids.contains(&c.id));
            }
            requests
        }
    ...
    fn default_endpoint_path() -> PathBuf {
        let pid = std::process::id();
        std::env::temp_dir().join(format!("wok-{pid}.port"))
    }
}
...
mod tests {
    ...
    fn test_remote_server_handles_multiple_clients() {
        ...
        client_one
            .write_all(br#"{"jsonrpc":"2.0","method":"wok.get_panes","id":1}"#)
        ...
        client_two
            .write_all(
                br#"{"jsonrpc":"2.0","method":"wok.notify","params":{"message":"hi"},"id":2}"#,
            )
        ...
        assert!(requests
            .iter()
            .any(|request| request.method == "wok.get_panes"));
        assert!(requests
            .iter()
            .any(|request| request.method == "wok.notify"));
