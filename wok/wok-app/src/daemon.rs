...
/// Daemon session summary for `wok list`.
#[derive(Debug, Clone)]
...
mod imp {
    ...
    fn daemon_terminal_env(name: &str) -> HashMap<String, String> {
        ...
        env.insert("WOK_SESSION".to_string(), name.to_string());
        env.insert(
            "WOK_SESSION_SOCKET".to_string(),
                        ...
                        Ok(message) => match message {
                            ClientMessage::Attach { .. } => {
                                session.attached_clients =
                                    session.attached_clients.saturating_add(1);
                            ...
                            ClientMessage::Detach { .. } => {
                                session.attached_clients =
                                    session.attached_clients.saturating_sub(1);
                                ServerMessage::Ack
                            }
                            ...
                            ClientMessage::ClosePane { pane_id } => session.close_pane(pane_id),
...
mod imp {
    ...
    fn daemon_terminal_env(name: &str) -> HashMap<String, String> {
        ...
        env.insert("WOK_SESSION".to_string(), name.to_string());
        env.insert(
            "WOK_SESSION_ENDPOINT".to_string(),
            session_socket_path(name).display().to_string(),
        );
        env
    }
