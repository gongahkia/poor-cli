//! Configuration file system: loads and provides typed access to Walk settings.
...
impl Default for WalkConfig {
    fn default() -> Self {
        Self {
            shell: walk_terminal::shell::detect_default_shell(),
            theme_path: None,
...
impl WalkConfig {
    ...
    ///
    /// Search order: `$WALK_CONFIG`, `~/.config/walk/config.toml`, `~/.walk.toml`.
    pub fn load() -> Self {
    ...
    pub fn config_dir() -> PathBuf {
        ...
        {
            std::env::var("HOME")
                .map(|h| PathBuf::from(h).join(".config").join("walk"))
                .unwrap_or_else(|_| PathBuf::from(".config/walk"))
        }
        #[cfg(windows)]
        {
            std::env::var("APPDATA")
                .map(|a| PathBuf::from(a).join("Walk"))
                .unwrap_or_else(|_| PathBuf::from("Walk"))
        }
    }
}
...
fn config_search_paths() -> Vec<PathBuf> {
    ...
    if let Ok(p) = std::env::var("WALK_CONFIG") {
    ...
    if let Ok(home) = std::env::var("HOME") {
        paths.push(PathBuf::from(home).join(".walk.toml"));
    }
