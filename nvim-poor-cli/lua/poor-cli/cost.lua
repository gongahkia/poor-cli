    ...
    local cost_spec = {
        ...
        verbs = {
            ...
            ["economy-preset"] = function(fargs)
                ...
                M.set_economy_preset({ preset = preset }, function(_, err) vim.schedule(function()
                    if err then notify(rpc.format_error(err), vim.log.levels.ERROR)
                    else notify("economy preset set: " .. preset, vim.log.levels.INFO) end
                end) end)
            end,
            history = function()
                ...
                M.get_cost_history({ limit = 20 }, function(result, err) vim.schedule(function()
                    ...
                    for _, e in ipairs(entries) do
                        ...
                        items[#items + 1] = { label = label, preview = vim.inspect(e), data = e }
                    end
            ...
            budget = function(fargs)
                local template = fargs[1]
                if not template or template == "" then
                    ...
                    M.apply_budget_template({ template = template }, function(_, err) vim.schedule(function()
                        if err then notify(rpc.format_error(err), vim.log.levels.ERROR)
                        else notify("budget template applied: " .. template, vim.log.levels.INFO) end
                    end) end)
                end
            end,
            compare = function(fargs)
                if #fargs < 2 then notify("usage: :PoorCLICost compare <provider> <model>", vim.log.levels.WARN); return end
                M.compare_model_cost({ provider = fargs[1], model = fargs[2] }, function(result, err) vim.schedule(function()
                    if err then notify(rpc.format_error(err), vim.log.levels.ERROR); return end
