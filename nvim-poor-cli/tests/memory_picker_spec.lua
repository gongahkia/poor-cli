...
describe("memory_picker", function()
    ...
    it("exposes open and cycle_sort (no legacy :PoorCLIMemoryPicker command)", function()
        local mp = require("poor-cli.memory_picker")
        assert.is_function(mp.open)
        assert.is_function(mp.cycle_sort)
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIMemoryPicker"])
    end)
end)
