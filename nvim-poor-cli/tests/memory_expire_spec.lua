...
describe("memory_expire", function()
    ...
    it("exposes M.open as the entry point (no legacy :PoorCLIMemoryExpire)", function()
        local me = require("poor-cli.memory_expire")
        assert.is_function(me.open)
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIMemoryExpire"])
    end)
end)
