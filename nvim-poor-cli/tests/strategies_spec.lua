...
describe("strategies", function()
    ...
    it("v6.2 exposes strategies as M functions; verbs now live under :PoorCLIAgent workflow-*", function()
        local s = require("poor-cli.strategies")
        assert.is_function(s.show)
        assert.is_function(s.set_reranker)
        assert.is_function(s.set_adaptive)
        local cmds = vim.api.nvim_get_commands({})
        assert.is_nil(cmds["PoorCLIStrategies"])
        assert.is_nil(cmds["PoorCLIRerankerStrategy"])
        assert.is_nil(cmds["PoorCLIAdaptivePruning"])
    end)
