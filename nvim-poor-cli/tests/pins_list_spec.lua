...
describe("pins_list", function()
    ...
    it("exposes M.open as the entry point (no legacy :PoorCLIPinsList command)", function()
        local pl = require("poor-cli.pins_list")
        assert.is_function(pl.open)
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIPinsList"])
    end)
end)
