...
describe("ux.palette", function()
    ...
    it("palette.open is the entry point (:PoorCLIHelp palette in v6.2)", function()
        local palette = require("poor-cli.ux.palette")
        assert.is_function(palette.open)
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIPalette"])
    end)
end)
...
describe("ux.home", function()
    ...
    it("home.go_home is the entry point (:PoorCLIHelp home in v6.2)", function()
        local home = require("poor-cli.ux.home")
        assert.is_function(home.go_home)
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIHome"])
    end)
end)
...
describe("panels dispatcher", function()
    ...
    it("v6.2 registers panel verbs on :PoorCLIAgent, not :PoorCLIPanel", function()
        -- Seed the target noun first so panels.setup() extends rather than queues.
        local spec = require("poor-cli.command_spec")
        spec.install("agent", { desc = "", verb_names = {}, verbs = {} }, function() end)
        require("poor-cli.panels").setup()
        assert.is_nil(vim.api.nvim_get_commands({})["PoorCLIPanel"])
        local agent = spec.get("agent")
        assert.is_not_nil(agent)
        assert.is_function(agent.verbs["panel-open"])
        assert.is_function(agent.verbs["panel-close"])
        assert.is_function(agent.verbs["panel-toggle"])
        assert.is_function(agent.verbs["runtime"])
    end)
