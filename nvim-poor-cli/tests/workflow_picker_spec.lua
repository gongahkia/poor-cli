...
describe("workflow picker", function()
    ...
    it("v6.2 workflows attach to :PoorCLIAgent as `workflow*`, not :PoorCLIWorkflows", function()
        local spec = require("poor-cli.command_spec")
        spec.install("agent", { desc = "", verb_names = {}, verbs = {} }, function() end)
        picker.setup()
        assert.is_nil(vim.api.nvim_get_commands({}).PoorCLIWorkflows)
        assert.is_function(spec.get("agent").verbs["workflow"])
        assert.is_function(spec.get("agent").verbs["workflow-strategies"])
    end)
