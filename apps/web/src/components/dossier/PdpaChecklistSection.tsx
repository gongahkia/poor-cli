  ...
  return (
    <section className="min-w-0 rounded-lg border border-border bg-card p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid gap-3">
        {items.map((item) => (
          <article className="min-w-0 rounded-md border border-border p-3" key={item.id}>
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <h3 className="break-words text-base font-semibold text-foreground">{item.title}</h3>
                <p className="mt-1 text-sm text-muted-foreground">{item.obligation}</p>
              </div>
              <span className={`w-fit shrink-0 rounded-md px-2.5 py-1 text-xs font-medium ${statusClassName(item.status)}`}>
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 break-words text-sm leading-6 text-foreground">{item.action}</p>
