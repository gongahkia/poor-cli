          ...
          return (
            <article className="rounded-md border border-border p-3" key={`${item.source}-${item.tool}`}>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                ...
                <p className="text-xs text-muted-foreground">Checked by Dude: {observedAt}</p>
              </div>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">{item.coverage}</p>
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                <span className="rounded-md bg-muted px-2 py-1">{getEvidenceTypeLabel(item)}</span>
                ...
                {item.sourceUrl !== undefined ? (
                  <a
                    className="rounded-md bg-muted px-2 py-1 underline-offset-4 hover:underline"
                    href={item.sourceUrl}
                    rel="noreferrer"
                    target="_blank"
                  >
