...
class ContextManager:
    ...
    async def select_context_files(
        ...
        selector = getattr(self, "_file_selector", None)
        if selector is not None:
            try:
                ranked = selector.rank(
                    files_to_include,
                    prompt=message,
                    pinned_paths=self._normalize_input_paths(pinned_files),
                    seed_paths=self._normalize_input_paths(explicit_files + pinned_files),
                )
                files_to_include = [candidate.file for candidate in ranked]
            except Exception:
                logger.debug("file selector ranking failed; falling back to priority sort", exc_info=True)
                files_to_include.sort(key=lambda f: (-f.priority, f.path))
        else:
            files_to_include.sort(key=lambda f: (-f.priority, f.path))

        limited_files = self._limit_preserving_pinned(
            files_to_include,
            effective_max_files,
            set(self._normalize_input_paths(pinned_files)),
        )
        ...
        excluded_entries = [
            ...
            for file_ctx in files_to_include
            if file_ctx.path not in {selected.path for selected in limited_files}
        ]
    ...
    def _limit_preserving_pinned(
        ...
        for file_ctx in files:
            if file_ctx.path in pinned_paths:
                selected.append(file_ctx)
                selected_paths.add(file_ctx.path)
        target = max(0, int(max_files or 0))
        for file_ctx in files:
            if file_ctx.path in selected_paths:
                continue
