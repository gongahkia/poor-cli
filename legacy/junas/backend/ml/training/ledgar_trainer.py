...
class LedgarTrainingConfig:
    model_name: str = MODEL_NAME
    output_dir: str = "models/ledgar-classifier"
...
def train_ledgar_model(config: LedgarTrainingConfig) -> dict[str, Any]:
...
def parse_args() -> LedgarTrainingConfig:
    parser = argparse.ArgumentParser(description="Train LEDGAR contract clause classifier")
    parser.add_argument("--model-name", type=str, default=MODEL_NAME)
    parser.add_argument("--output-dir", type=str, default="models/ledgar-classifier")
...
def main() -> int:
    config = parse_args()
    result = train_ledgar_model(config)
    print(result)
    return 0
