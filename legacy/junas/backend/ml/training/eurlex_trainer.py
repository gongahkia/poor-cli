...
class EurlexTrainingConfig:
    model_name: str = MODEL_NAME
    output_dir: str = "models/eurlex-classifier"
    ...
    seed: int = 42
    run_name: str = "eurlex-legal-bert"
    register_to_benchmarks: bool = False
    benchmark_api_base_url: str = "http://localhost:8000"


def train_eurlex_model(config: EurlexTrainingConfig) -> dict[str, Any]:
...
def parse_args() -> EurlexTrainingConfig:
    parser = argparse.ArgumentParser(description="Train EUR-LEX multi-label classifier")
    parser.add_argument("--model-name", type=str, default=MODEL_NAME)
    parser.add_argument("--output-dir", type=str, default="models/eurlex-classifier")
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--max-length", type=int, default=512)
    parser.add_argument("--threshold", type=float, default=0.5)
    parser.add_argument("--grad-accum", type=int, default=4)
    parser.add_argument("--run-name", type=str, default="eurlex-legal-bert")
...
def main() -> int:
    config = parse_args()
    result = train_eurlex_model(config)
    print(result)
    return 0
