...
def _isolate_probe_cache_path(monkeypatch, tmp_path):
    monkeypatch.setenv(
        "POORCLI_PROVIDER_PROBE_CACHE_PATH",
        str(tmp_path / "provider_probe_cache.json"),
    )
    _reset_probe_cache()
    yield
    _reset_probe_cache()
...
def test_probe_providers_loads_disk_cache_without_reprobe(monkeypatch, tmp_path):
    config = Config()
    manager = _ConfigManagerStub()
    _reset_probe_cache()
    monkeypatch.setenv("POORCLI_PROVIDER_PROBE_CACHE_PATH", str(tmp_path / "provider_probe_cache.json"))
    monkeypatch.setenv("POORCLI_PROVIDER_PROBE_ALL_LOCAL", "1")
