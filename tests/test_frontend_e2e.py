...
def test_case_query_loads_review_panel(browser_page, viewer_base_url: str) -> None:
    _mock_editor_backend(browser_page)
    browser_page.route(
        "**/api/sync-layout",
        lambda route: route.fulfill(status=200, content_type="application/json", body=json.dumps({"ok": True})),
        ...
        "compliance_findings": [
            {
                ...
                "reason": "Cannot remove shelter wall.",
            }
        ],
        "approval_state": {"decision": "pending"},
        "vendor_handoff": None,
    }
    browser_page.route(
        "**/case-demo.json",
    ...
    browser_page.goto(f"{viewer_base_url}/viewer/editor.html?case=/case-demo.json")
    browser_page.wait_for_selector("#case-review-section", state="visible", timeout=6000)

    panel_text = browser_page.locator("#case-review-section").inner_text()
