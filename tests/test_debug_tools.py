...
def test_set_breakpoint_without_dap_returns_error():
    ctx = _ctx(plugins={})
...
def test_set_breakpoint_with_dap_fires_notification():
    log = []
    ctx = _ctx(plugins={"dap": True}, notify_log=log)
    r = _run(
...
def test_step_and_continue_fire_notifications():
    ...
    methods = [m for m, _ in log]
    assert methods == ["integration.dap.step", "integration.dap.continue"]
