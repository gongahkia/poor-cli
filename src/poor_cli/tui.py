...
from .run_records import diff_runs
...
def run_tui(store_dir: Path, run_id: str | None = None) -> None:
    ...
    class PoorCliTui(App[None]):
        ...
        #topbar { height: 1; background: #06242c; color: #d9fbff; content-align: center middle; }
        #panes { height: 1fr; } #left { width: 48%; } #right { width: 52%; }
        #provider, #graph, #transcript, #artifacts, #activity {
            width: 1fr; border: round #52666d; padding: 0 1; background: #020406; color: #b9cccc;
        }
        #provider { height: 8; border-title-color: #89dd88; } #graph { height: 1fr; border-title-color: #f6c177; }
        #transcript { height: 10; border-title-color: #7aa2f7; } #artifacts { height: 10; border-title-color: #c099ff; }
        #activity { height: 1fr; border-title-color: #ff9e64; } #composer { height: 3; border: none; }
        #goal { height: 3; border: tall #2f81f7; background: #020406; color: #d9fbff; }
        #help { height: 1; background: #03070a; color: #7b8c8d; content-align: right middle; }
        """

        def compose(self) -> ComposeResult:
            yield Static(f"poor-cli tui  {datetime.now().strftime('%H:%M:%S')}", id="topbar")
            ...
            with Vertical(id="composer"):
                yield Input(placeholder="run --dry <goal> | replay <run_id> | open PLAN.md | diff recent | route set ...", id="goal")
        ...
        def on_input_submitted(self, event: Input.Submitted) -> None:
            ...
            event.input.value = ""
            try:
                result = handle_tui_command(store_dir, command, repo_path=Path.cwd(), current_run_id=self.selected_run_id)
                self.selected_run_id = result.run_id or self.selected_run_id
                self.query_one("#transcript", Static).update(result.message)
...
def handle_tui_command(
    store_dir: Path, command: str, *, repo_path: Path | None = None, current_run_id: str | None = None
) -> TuiCommandResult:
    text = command.strip()
    if not text:
    ...
    if text.startswith("open "):
        return _handle_open(store_dir, text.split(maxsplit=1)[1].strip(), current_run_id)
    if text.startswith("diff "):
        return _handle_diff(store_dir, text.split(maxsplit=1)[1].strip())
...
def _handle_run(store_dir: Path, command: str, repo_path: Path) -> TuiCommandResult:
    ...
    budget = Budget()
    with closing(RunStore(store_dir)) as store:
...
def _handle_open(store_dir: Path, target: str, current_run_id: str | None) -> TuiCommandResult:
    parts = target.split(maxsplit=1)
    run_id, name = (parts[0], parts[1]) if len(parts) == 2 and parts[0].startswith("run_") else (current_run_id, target)
    if not run_id or not name:
        raise RuntimeError("open requires a selected run or: open <run_id> <artifact>")
    with closing(RunStore(store_dir)) as store:
        base = (store.root / "runs" / run_id / "artifacts").resolve()
        candidate = (base / name).resolve()
        if base in candidate.parents and candidate.is_file():
            return TuiCommandResult(run_id, candidate.read_text(encoding="utf-8", errors="replace"))
        for artifact in store.list_artifacts(run_id):
            if name in {artifact["artifact_id"], artifact["sha256"], artifact["kind"]}:
                return TuiCommandResult(run_id, store.artifact_payload(artifact["artifact_id"]).decode("utf-8", errors="replace"))
    raise RuntimeError(f"artifact not found: {name}")


def _handle_diff(store_dir: Path, target: str) -> TuiCommandResult:
    with closing(RunStore(store_dir)) as store:
        runs = store.list_runs()
        parts = target.split()
        if parts == ["recent"] or not parts:
            if len(runs) < 2:
                raise RuntimeError("diff recent requires at least two runs")
            run_a, run_b = runs[1]["run_id"], runs[0]["run_id"]
        elif len(parts) == 2:
            run_a, run_b = parts
        else:
            raise RuntimeError("diff requires: diff recent or diff <run_a> <run_b>")
        payload = diff_runs(store, run_a, run_b)
    lines = [f"diff {run_a} {run_b} changed={payload['changed']}", json.dumps(payload["summary"], sort_keys=True)]
    lines.extend(f"{change['section']} {change['classification']}" for change in payload["changes"])
    return TuiCommandResult(run_b, "\n".join(lines))
...
def render_artifact_panel(store_dir: Path, run_id: str | None) -> str:
    if not run_id:
