        ...
        for (tid, tr) in &entity.timeline_appearances {
            if let Some(tl) = world.timelines.get(tid) {
                if let (Some(ref tl_start), Some(ref tl_end)) = (&tl.start, &tl.end) {
                    ...
                    if ent_s < tl_s {
                        errors.push(format!("entity '{}' appears at {} before timeline '{}' starts at {}", entity.name, ent_s, tl.name, tl_s));
                    }
                    if ent_e > tl_e {
                        warnings.push(format!("entity '{}' extends to {} beyond timeline '{}' end at {}", entity.name, ent_e, tl.name, tl_e));
                    }
                }
            }
        }
            ...
            if let Some(parent) = world.timelines.get(&parent_id) {
                if let (Some(ref ps), Some(ref pe)) = (&parent.start, &parent.end) {
                    ...
                    if fp < ps.to_ordinal() || fp > pe.to_ordinal() {
                        errors.push(format!("fork point of '{}' at {} is outside parent '{}' bounds", tl.name, fp, parent.name));
                    }
                }
            }
        }
            ...
            if let Some(target) = world.timelines.get(&target_id) {
                if let (Some(ref ts), Some(ref te)) = (&target.start, &target.end) {
                    ...
                    if mp < ts.to_ordinal() || mp > te.to_ordinal() {
                        errors.push(format!("merge of '{}' into '{}' at {} is outside target bounds", tl.name, target.name, mp));
                    }
                }
            }
        }
    ...
    for tl in world.timelines.values() {
        if tl.kind == TimelineKindModel::Loop {
            if let Some(ref lc) = tl.loop_config {
                if lc.count <= 0 {
                    warnings.push(format!("loop '{}' has non-positive iteration count {}", tl.name, lc.count));
                }
                if lc.entry_time.to_ordinal() >= lc.exit_time.to_ordinal() {
                    errors.push(format!("loop '{}' entry >= exit (infinite loop potential)", tl.name));
                }
            } else {
                warnings.push(format!("loop '{}' has no loop config (no exit condition)", tl.name));
            }
        }
    }
}
...
fn validate_relationship_temporal_scope(world: &World, errors: &mut Vec<String>) {
    for rel in &world.relationships {
        if let Some(ref scope) = rel.temporal_scope {
            let src_name = world.entity_by_id(rel.source_entity_id).map(|e| e.name.as_str()).unwrap_or("?");
            let tgt_name = world.entity_by_id(rel.target_entity_id).map(|e| e.name.as_str()).unwrap_or("?");
            if let Some(src) = world.entity_by_id(rel.source_entity_id) {
                let src_overlaps = src.timeline_appearances.iter().any(|(_, tr)| tr.overlaps(scope));
                if !src_overlaps {
            ...
            if let Some(tgt) = world.entity_by_id(rel.target_entity_id) {
                let tgt_overlaps = tgt.timeline_appearances.iter().any(|(_, tr)| tr.overlaps(scope));
                if !tgt_overlaps {
        ...
        while let Some(parent_tl) = world.timelines.get(&current) {
            if let Some(pid) = parent_tl.parent_id {
                if !visited.insert(pid) {
                    errors.push(format!("timeline hierarchy cycle detected involving '{}'", tl.name));
                    break;
                }
                current = pid;
            } else {
...
fn validate_parallel_consistency(world: &World, warnings: &mut Vec<String>) {
    let parallels: Vec<_> = world.timelines.values()
