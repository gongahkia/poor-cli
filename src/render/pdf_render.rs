...
pub fn render_pdf_vector(
    ...
    for tl in &layout.timelines {
        ...
        let y_top = height_mm as f64 - PDF_MARGIN as f64 - (tl.lane_start as f64 * PDF_LANE_HEIGHT as f64);
        ...
        let rect_points = vec![
            ...
            (Point::new(Mm((x + w) as f32), Mm((y_top - h) as f32)), false),
            (Point::new(Mm(x as f32), Mm((y_top - h) as f32)), false),
        ];
        let line = Line { points: rect_points, is_closed: true, ..Default::default() };
        ...
        // Timeline label
        current_layer.use_text(&tl.name, PDF_FONT_SIZE, Mm(x as f32 + 1.0), Mm(y_top as f32 - 3.0), &font);
    }

    // Draw entity bars
    for ent in &layout.entities {
        let x = PDF_MARGIN as f64 + (ent.x_start - vp.time_start) * scale_x;
        let y_top = height_mm as f64 - PDF_MARGIN as f64 - (ent.lane as f64 * PDF_LANE_HEIGHT as f64);
        let w = ((ent.x_end - ent.x_start) * scale_x).max(2.0);

        let (r, g, b) = parse_hex_color(
            theme.entity_colors.get(&ent.entity_type).map(|s| s.as_str()).unwrap_or("#888888")
        );
        ...
        let rect_points = vec![
            (Point::new(Mm(x as f32), Mm(y_top as f32)), false),
            (Point::new(Mm((x + w) as f32), Mm(y_top as f32)), false),
            (Point::new(Mm((x + w) as f32), Mm((y_top as f32) - PDF_BAR_HEIGHT)), false),
            (Point::new(Mm(x as f32), Mm((y_top as f32) - PDF_BAR_HEIGHT)), false),
        ];
        let line = Line { points: rect_points, is_closed: true, ..Default::default() };
        current_layer.add_line(line);
        ...
        let label = if ent.name.len() > 25 { format!("{}...", &ent.name[..22]) } else { ent.name.clone() };
    ...
    for edge in &layout.edges {
        ...
        let src_y = height_mm as f64 - PDF_MARGIN as f64 - (edge.source_lane as f64 * PDF_LANE_HEIGHT as f64) - PDF_BAR_HEIGHT as f64 / 2.0;
        let tgt_y = height_mm as f64 - PDF_MARGIN as f64 - (edge.target_lane as f64 * PDF_LANE_HEIGHT as f64) - PDF_BAR_HEIGHT as f64 / 2.0;
        ...
        if !edge.label.is_empty() {
            ...
            current_layer.use_text(&edge.label, PDF_FONT_SIZE - 2.0, Mm(mid_x), Mm(mid_y + 1.5), &font);
        }
    }
...
fn render_pdf_raster(svg_data: &str, output: &Path, width_mm: f32, height_mm: f32) -> Result<(), String> {
    let (doc, page1, layer1) = PdfDocument::new("Seuss Timeline", Mm(width_mm), Mm(height_mm), "Timeline");
    let current_layer = doc.get_page(page1).get_layer(layer1);
