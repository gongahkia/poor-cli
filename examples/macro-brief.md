...
```text
Give me a Singapore macro snapshot with USD/SGD, SORA, and the best GDP and CPI entrypoints.
```
...
Raw macro setup means querying multiple MAS series and then separately discovering which SingStat datasets are the right GDP and CPI entrypoints for the next step.
...
```json
{
  ...
  "summary": [
    ...
    { "label": "GDP table ID", "value": "gdp", "source": "SingStat" }
  ],
  "evidence": [
    { "label": "FX rows", "value": 1, "source": "MAS" },
    { "label": "GDP candidates", "value": 1, "source": "SingStat" }
  ],
  "gaps": [],
  "provenance": [
