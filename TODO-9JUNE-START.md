...
- **Tests.** 44 new tests across `tests/test_case_{ingest,compliance,design_agent,revise_loop,http_server,vendor_handoff}.py` — all passing. Includes the SPEC Appendix A 10-step round-trip as an integration test, HTTP boundary coverage, and cached handoff packet generation. Full repo suite: 96 passed, 1 skipped, 1 xfailed. Ruff clean.
