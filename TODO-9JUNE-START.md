...
- **Stage 1 contract (`SPEC-HTTP-CASE.md` at repo root).** Locks: case payload schema (extends `corpus/library/*.json` shape with `case_id`, `case_schema_version`, `design_status` enum, `revise_count`, `compliance_findings[]` with `machine_hint`, `approval_state`, `vendor_handoff`, reserved keys `pinned_proposal_id` + `vendor_cache_key`); core lifecycle endpoints plus Stage-1 approval/handoff routes; revise-loop policy (N=3 default); two compliance rules (`structural_wall_protected`, `walkway_accessibility`); local SQLite persistence + optional Bearer auth; Appendix A 10-step round-trip.
...
- **Stage 1 HTTP service** (`src/haus/case/http_server.py`). Starlette/Uvicorn wrapper over the case step functions: `POST /case`, `POST /case/{id}/design`, `POST /case/{id}/compliance`, `POST /case/{id}/revise`, `GET /case/{id}`, temporary `PATCH /case/{id}/approval`, and `POST /case/{id}/handoff`. It persists Cases through SQLite by default and supports optional Bearer auth. CLI entrypoint: `haus case-server`.
- **Vendor/Handoff Agent v0** (`src/haus/case/vendor_handoff.py`). Cache-first vendor resolver with TinyFish live search on cache miss and deterministic stub fallback when no key/search result exists; generates a per-case `packet.zip` containing `handoff.json` + `summary.md`, populates `vendor_handoff`, and transitions `approved -> handoff_complete`. Demo cache lives at `tests/fixtures/vendors/demo_hdb_renovation.json`; HTTP route: `POST /case/{id}/handoff`.
...
- **Vendor agent is cache-first.** The v0 handoff agent uses cached vendors first, runs TinyFish vendor search on cache miss when `TINYFISH_API_KEY` is set, writes successful live results back to the cache directory, and keeps the deterministic live-search stub as the no-key/error fallback.
...
7. **HDB renovation compliance:** v0 ruleset exists: structural/shelter wall removal/move/rotate/resize + walkway accessibility. Broader HDB rules are still deferred.
...
11. **Contractor handoff:** v0 is cache-first, runs TinyFish live vendor search on cache miss when configured, writes successful live results back to cache, and falls back to a deterministic stub when no live provider is available.
...
| **Vendor/Handoff Agent** | Matches design to vendors; produces handoff package | Thin; TinyFish live search with cached fallback | Deliberately thin — proves the handoff exists, not a full CRM |
...
- [x] Implement **Vendor/Handoff Agent** (thin): TinyFish live search where available, **pre-seeded cache** for the demo (see §6 task E). — **v0 done:** `src/haus/case/vendor_handoff.py`; pre-seeded cache at `tests/fixtures/vendors/demo_hdb_renovation.json`; HTTP route `POST /case/{id}/handoff`; writes `packet.zip`, writes successful live search results back to the vendor cache, and populates `vendor_handoff`.
...
**E. Vendor cache for a reliable demo.** For the *recorded* demo, pre-seed the vendor cache so the contractor-handoff step never depends on a live TinyFish call. Where does the cache live, what's its schema, and how does the handoff agent read from it (cache-first, live-search-fallback)? — **RESOLVED.** Cache directory: `tests/fixtures/vendors/` for demo fixtures, configurable through `HAUS_VENDOR_CACHE_DIR` / `--vendor-cache-dir`; default cache key: `demo_hdb_renovation`; schema: `{cache_key, description, retrieved_at, vendors: [{vendor_id, vendor_name, specialties, service_area, contact, packet_template}]}`. Implementation: `src/haus/case/vendor_handoff.py`; reads cache-first, runs TinyFish search on cache miss when `TINYFISH_API_KEY` is set, writes successful live results back to cache, falls back to a deterministic live-search stub when needed, writes `packet.zip`, and sets `vendor_handoff`.
...
| **Compliance ruleset stays too narrow** | The money-shot works, but judges may ask whether broader HDB rules are covered | Say v0 covers protected-wall removal/move/rotate/resize + accessibility; frame broader permit/scheduling/fire-safety rules as extensibility points. |
