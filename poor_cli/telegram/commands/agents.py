...
from poor_cli.telegram.keyboards import agent_keyboard
...
async def _handle_agents(bot, update: Any, context: Any) -> None:
    ...
    try:
        if sub in ("list", "create", "start", "cancel", "logs", "result"):
            result = await _chat_cmd(core, f"/agents {sub} {rest}".strip())
            pages = fmt.paginate(result or f"agents {sub} done")
            for page in pages:
                await update.effective_message.reply_text(page)
        else:
            await update.message.reply_text(
                "usage: /agents [list|create <prompt>|start <id>|cancel <id>|logs <id>|result <id>]"
            )
...
def register(app, bot):
    async def handler(update, context):
        await _handle_agents(bot, update, context)
    app.add_handler(CommandHandler("agents", handler))
