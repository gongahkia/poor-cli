...
`poor_cli/memory_reranker.py` ships `mmr(query_vec, candidates, lambda_=0.7, k=5)` and `rerank_semantic_hits(query_vec, hits, embeddings, strategy, ...)`. MMR trades relevance (`lambda_=1.0`) against diversity (`lambda_=0.0`) using cosine sim across existing vectors — no new dependencies. Default 0.7 slightly favors relevance while injecting diversity to dodge the near-duplicate trap. Cross-encoder strategy shipped 2026-04-15 with graceful `sentence-transformers` ImportError fallback to MMR, plus `score_order` trivial strategy. User-selectable at runtime via `:PoorCLIRerankerStrategy` (values: `mmr` | `cross_encoder` | `score_order`); persisted to `.poor-cli/strategies.json`. `hybrid_search` consults the setting. Tested in `tests/test_memory_reranker.py`, `tests/test_memory_reranker_strategies.py`, `tests/test_ux_strategies.py`.

### MH8. Memory access-recency telemetry — DONE 2026-04-14
`MemoryEntry.hit_count` and `last_accessed_at` now live in frontmatter (backward-compatible: missing fields default to `0` / `created_at`). `MemoryManager.search()` and `MemoryManager.get()` take a `record_hit`/`record_hits` flag (default true) that calls `entry.touch()` and persists the updated frontmatter to disk. `list_all()` stays read-only so enumeration doesn't inflate counts. Tested in `tests/test_memory_provenance.py`. `:PoorCLIMemoryPicker` nvim surface shipped 2026-04-15 (`nvim-poor-cli/lua/poor-cli/memory_picker.lua`): sorts by hits / recency / name; `:PoorCLIMemoryPickerSort` cycles modes.
...
`PruningPolicy.soft_pin_evict_factor` (default 1.05) gates soft-pin eviction: turns with `pinned: "soft"` enter the eviction pool only when `current_tokens > target * 1.05`. Legacy `pinned: true` keeps hard-pin semantics. `ScoredTurn.soft_protected` is surfaced on both the dataclass and the annotated metadata. Tested in `tests/test_history_soft_pin.py`. Nvim surface shipped 2026-04-15: `poor_cli/turn_pin_overlay.py` persists per-repo pin states at `.poor-cli/turn_pins.json`, `poor-cli/setTurnPin` + `poor-cli/listTurnPins` RPCs, `history_pruning.prune(turn_pin_overlay=...)` merges into metadata, and chat-buffer keymap `gp` cycles state (none → soft → hard → none) with badge render via `nvim-poor-cli/lua/poor-cli/turn_pin.lua`.

### CB3. Per-tool-success adaptive history pruning — DONE 2026-04-14
`poor_cli/tool_success_tracker.py` ships `ToolSuccessTracker` with a JSON-backed rolling counter at `.poor-cli/tool_success_cache.json`. `tool_weight_multiplier(name, min_samples=5)` maps rolling success rate to a multiplier in `[0.5, 1.5]`; neutral (1.0) before `min_samples` observations. `HistoryPruner(tool_success_tracker=...)` applies the multiplier only when `PruningPolicy.adaptive_tool_scoring=True`. Call-site wiring verified 2026-04-15: `core_tool_dispatch._execute_tool_internal` calls `tracker.record(tool_name, success)` on both the success and exception paths (auto-persists every 25 records). `adaptive_tool_scoring_override` (None/True/False) and runtime toggle `:PoorCLIAdaptivePruning {auto|on|off}` let users force the behavior regardless of default; setting persists to `.poor-cli/strategies.json`. Tested in `tests/test_tool_success_tracker.py` + `tests/test_adaptive_scoring_override.py`.
...
Documented the caching matrix across all 11 providers in `poor_cli/providers/base.py` module docstring. Anthropic stays the only provider with explicit `cache_control` markers; others rely on their native server-side caches (OpenAI implicit prefix, Gemini `cachedContent` opt-in, vLLM/SGLang/TGI automatic). Per-provider cache telemetry shipped 2026-04-15: `_track_cost(provider=...)` accumulates hits/misses/read_tokens/write_tokens/savings_usd/cost_usd into `_session_provider_cache_stats`, and `get_session_summary().cache.by_provider` surfaces the breakdown. `:PoorCLICostDashboard` renders a "Per-provider cache" section when populated. Tested in `tests/test_per_provider_cache_telemetry.py`.
