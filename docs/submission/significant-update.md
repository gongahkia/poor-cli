...
- Added Shield-governed Splunk MCP proxy tools: `splunk_search`, `splunk_list_indexes`, and `splunk_list_saved_searches`.
...
- Added runtime output defense at the Shield enforcement seam: credential/PII-shaped output redaction, prompt-injection neutralization, and optional critical-finding block mode.
- Added audit persistence for runtime findings and dual hashes: raw upstream output hash plus post-redaction output hash.
- Extended the dashboard Shield audit panel with finding counts, reason codes, severity/action summaries, and short hash evidence.
