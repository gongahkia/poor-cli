...
This repository is **two surfaces in one**:
1. **Dude MCP** (`packages/mcp-server`), the backend/runtime exposing 105 `sg_*` tools across 38 cataloged Singapore data and advisory families.
...
Namespace note: `sg_*` tool names, `sg://...` resources, and `SG_API_*` / `SG_APIS_*` environment variables are stable Singapore-data contract namespaces. They are not the product brand.
