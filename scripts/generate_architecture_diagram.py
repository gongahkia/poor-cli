...
ICON_DIR = Path("/tmp/seuss-diagram-icons")
...
def main() -> None:
    graph_attr = {
        ...
        "fontsize": "22",
        "labelloc": "t",
        "label": "Seuss Haskell Architecture",
    }
    node_attr = {
        "fontname": "Helvetica",
        "fontsize": "13",
    ...
    with Diagram(
        ...
        with Cluster("Language Core", graph_attr={"bgcolor": "#f8fafc", "color": "#cbd5e1"}):
            ...
            ast = node("AST\nSeuss.Lang.AST", "ast.png", "haskell", "AST", "#4c1d95")
            evaluator = node("Evaluator\nSeuss.Core.Eval", "eval.png", "haskell", "EVAL", "#5b21b6")
            world = node(
        ...
        csv >> Edge(label="emit .seuss") >> parser
        gedcom >> Edge(label="emit .seuss") >> parser
        jsonld >> Edge(label="emit .seuss") >> parser
